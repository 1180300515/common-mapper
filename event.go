...
func CreateMessageTwinUpdate(name []string, valueType []string, value []string) (msg []byte, err error) {
	...
	for i:=0 ; i < len(name) ; i++{
		updateMsg.Twin[name[i]] = &MsgTwin{}
		updateMsg.Twin[name[i]].Actual = &TwinValue{Value: &value[i]}
		updateMsg.Twin[name[i]].Metadata = &TypeMetadata{Type: valueType[i]}
	}
